...
const getHost = async(email: string) => {
  const data = await axios.get(`http://ec2-3-16-183-72.us-east-2.compute.amazonaws.com/host/${email}`)
  return data.data
};
...
export const putHost = (hostId: number, data: GetHost) => {
  return axios.put(`http://ec2-3-16-183-72.us-east-2.compute.amazonaws.com/host/update/${hostId}`, data)
};

export const postHost = (data: Host) => {
  return axios.post("http://ec2-3-16-183-72.us-east-2.compute.amazonaws.com/host/register", data)
};
